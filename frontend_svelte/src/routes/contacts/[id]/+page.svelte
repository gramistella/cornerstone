<script lang="ts">
    ...
    let contact: ContactDto = data.contact;
	let errorMessage = '';

	async function handleSubmit() {
		errorMessage = '';
		try {
			await putApi(`contacts/${contact.id}`, contact);
			await goto('/contacts', { invalidateAll: true });
		} catch (error) {
			errorMessage = (error as Error).message || 'Failed to update contact.';
...
<div class="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
	<h2 class="text-2xl font-bold mb-6 text-center">Edit Contact</h2>
